...
find(SelectorRecord, ProjectorRecord, Skip, BatchSize) ->
    ...
    ?DEBUG("~p:find ~p Collection=~p, Selector=~p, Projector=~p, Skip=~p, BatchSize=~p~n", [?MODULE, ?LINE, Collection, Selector, Projector, Skip, BatchSize]),
...
insert(Record) ->
	...
	%mongo:insert(Collection, Document).
        %io:format("~p:insert ~p Collection=~p, Document=~p~n", [?MODULE, ?LINE, Collection, Document]),
