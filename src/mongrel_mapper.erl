...
get_type(Record) ->
    ?DEBUG("~p:get_flattened_map ~p Recrod = ~p", [?MODULE, ?LINE, Record]),
...
map_selector(SelectorRecord) when is_tuple(SelectorRecord) ->
    ?DEBUG("~p:map_selector ~p SelectorRecord = ~p ~n", [?MODULE, ?LINE, SelectorRecord]), 
...
map_advanced_selector({true, RecordName, Result}, []) ->
	R = {RecordName, list_to_tuple(Result)},
        %R = {octopus_log, {'$orderby', {time_created,-1}, '$query', {'$or', {source_id, <<"cc8093b1ca5a79287a1c8e2e58c04c3455c7c570">>, time_created, {'$lt',{0,-30600,0}}}}}},
        %R = {octopus_log, {'$orderby', {time_created,-1}, '$query', {'$or', [{source_id, <<"cc8093b1ca5a79287a1c8e2e58c04c3455c7c570">>}, {time_created, {'$lt',{0,-30600,0}}}]}}},
        ?DEBUG("~p:map_advanced_selector ~p R= ~p", [?MODULE, ?LINE, R]),
        R;
map_advanced_selector({false, RecordName, Result}, ['$self_query', Query | Tail]) ->
	?DEBUG("~p:map_advanced_selector ~p RecordName = ~p, Query = ~p, Tail = ~p", [?MODULE, ?LINE, RecordName, Query, Tail]),
	map_advanced_selector({true, RecordName, Result ++ ['$query', Query]}, Tail);
...
%%map_advanced_selector({GotType, undefined, Result}, ['collection' = Key, Value | Tail]) ->
%%	RecordName = get_type(Value),
%%	?DEBUG("~p:map_advanced_selector ~p RecordName = ~p,[Key, Value]= ~p,Tail = ~p", [?MODULE, ?LINE, RecordName, [Key, Value], Tail]),
%%	map_advanced_selector({GotType, RecordName, Result}, Tail);

map_advanced_selector({GotType, undefined, Result}, [Key, Value | Tail]) ->
	RecordName = get_type(Value),
	?DEBUG("~p:map_advanced_selector ~p RecordName = ~p,[Key, Value]= ~p,Tail = ~p", [?MODULE, ?LINE, RecordName, [Key, Value], Tail]),
...
map_advanced_selector({GotType, RecordName, Result}, [Key, Value | Tail]) ->
        ?DEBUG("~p:map_advanced_selector ~p RecordName = ~p,[Key, Value]= ~p,Tail = ~p", 
                 [?MODULE, ?LINE, RecordName, [Key, Value], Tail]),
	map_advanced_selector({GotType, RecordName, Result ++ [Key, get_flattened_map(Value)]}, Tail);

map_advanced_selector(Test1, Test2) ->
    ?DEBUG("~p:map_advanced_selector ~p Test1 = ~p, Test2 = ~p", [?MODULE, ?LINE, Test1, Test2]).
